...
import { Download, Edit, GripVertical, Trash2, Lock, Unlock, XCircle } from "lucide-react";
...
export type ScaleUtility = Tables<'balancas_utilitarios'>;

interface ScaleUtilityCardProps {
  utility: ScaleUtility;
  isAdmin: boolean;
  onEdit: (utility: ScaleUtility) => void;
  onDelete: (utility: ScaleUtility) => void;
...
export const ScaleUtilityCard = ({
  ...
  isDragging,
  showHiddenInfo,
}: ScaleUtilityCardProps) => {
  const { hideHiddenInfo } = useHiddenInfo();
    ...
    <>
      ...
      >
        ...
        <CardContent>
          ...
          {utility.hidden_info && utility.hidden_info.trim() !== '' && showHiddenInfo && (
